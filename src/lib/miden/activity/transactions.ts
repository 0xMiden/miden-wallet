...
export const completeConsumeTransaction = async (id: string, result: TransactionResult) => {
  ...
  const sender = note.metadata().sender().toBech32(NetworkId.Mainnet, AccountInterface.BasicWallet);
  ...
  const faucetId = asset.faucetId().toBech32(NetworkId.Mainnet, AccountInterface.BasicWallet);
