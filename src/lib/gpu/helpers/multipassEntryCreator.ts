 ...
 */
export const multipassEntryCreatorReuseBuffers = async (
  gpu: GPUDevice,
  ...
  for (let passIndex = 0; passIndex < passes.length; passIndex++) {
    ...
    const bindGroupLayout = createBindGroupLayout(gpu, inputData.inputBuffers, resultData.resultBuffers);

    // Create bind group
    const bindGroup = createBindGroup(gpu, bindGroupLayout, inputData.inputBuffers, resultData.resultBuffers);
  ...
  commandEncoder.copyBufferToBuffer(finalResultBuffer, 0, gpuReadBuffer, 0, entryInfo.outputSize);
  // }
