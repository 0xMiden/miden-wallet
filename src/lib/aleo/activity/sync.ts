import { getSerialNumbers, getRecordsByTransitionAndIndex, getProgram, getRecordMetadata } from 'lib/aleo-chain';
import { AleoProgram, extractMicrocredits, parseAleoProgram } from 'lib/aleo-chain/helpers';
import { IRecordMetadata } from 'lib/aleo-chain/rpc-types';
import { createAddressToKeysMap } from 'lib/aleo-sdk-utils';
import { scanRecordsDirect } from 'lib/aleo-worker/scanRecordsDirect';
import { scanRecordsGpu } from 'lib/aleo-worker/scanRecordsGpu';
...
export async function syncOwnedRecords(
  ...
  const addressToKeysMap = createAddressToKeysMap(keys);
