...
import { AleoAccountTokenBalance } from 'lib/aleo-chain';

interface infiniteListProps {
  getCount: (address: string) => Promise<number>;
  getItems: (address: string, page?: number) => Promise<Array<AleoAccountTokenBalance>>;
}

export const useInfiniteList = ({ getCount, getItems }: infiniteListProps) => {
  const address = 'account.publicKey';
  const [items, setItems] = useState<Array<AleoAccountTokenBalance>>([]);
