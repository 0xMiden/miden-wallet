...
const icons: Record<IdenticonProps['type'], Avatars<{}>> = {
...
const Identicon: FC<IdenticonProps> = ({
  ...
  const backgroundImage = useMemo(() => {
    const key = `${type}_${publicKey}_${size}`;
    if (cache.has(key)) {
      ...
      const opts =
        type === 'initials'
          ? {
              ...
              radius: 50,
              fontSize: estimateOptimalFontSize(publicKey.slice(0, MAX_INITIALS_LENGTH).length)
            }
          : basicOpts;
      let imgSrc = '';
      imgSrc = icons[type].create(publicKey, opts);
      const bi = `url('${imgSrc}')`;
      cache.set(key, bi);
      return bi;
    }
