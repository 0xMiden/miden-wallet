...
import { useConsume, useImportNote, useMiden } from '@miden-sdk/react';
...
export const Receive: React.FC<ReceiveProps> = () => {
  ...
  const { consume: consumeNotes } = useConsume();
  const { importNote } = useImportNote();
  const { client: sdkClient, runExclusive } = useMiden();
  ...
  const unclaimedNotesRef = useRef(unclaimedNotes);
  unclaimedNotesRef.current = unclaimedNotes;
  ...
  const handleClaimAll = useCallback(async () => {
    if (unclaimedNotesRef.current.length === 0) return;

    // Refresh the claimable notes list to avoid race conditions with auto-consume
    await mutateClaimableNotes();

    // Read latest unclaimed notes via ref (updated by re-render after refetch)
    const freshUnclaimed = unclaimedNotesRef.current;
    if (freshUnclaimed.length === 0) {
      return;
    }

    const noteIds = freshUnclaimed.map(n => n!.id);
    ...
    try {
      ...
      console.error('[ClaimAll] Failed to consume notes:', err);
      setFailedNoteIds(new Set(noteIds));
    } finally {
      setClaimingNoteIds(new Set());
    }
  }, [account.publicKey, mutateClaimableNotes, consumeNotes]);
      ...
      reader.onload = async (e: ProgressEvent<FileReader>) => {
        try {
          if (e.target?.result instanceof ArrayBuffer) {
            ...
            const noteId = await importNote(noteBytesAsUint8Array);
...
export const ConsumableNoteComponent = ({
  ...
  const handleConsume = useCallback(async () => {
    ...
    try {
      // Use SDK's useConsume() — handles execute, prove, submit, and sync
      await consume({
        accountId: account.publicKey,
        notes: [note.id]
      });
  ...
  }, [account.publicKey, consume, mutateClaimableNotes, note.id]);
