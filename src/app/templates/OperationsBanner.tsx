...
const OperationsBanner = memo<OperationsBannerProps>(
  ({ jsonViewStyle, opParams, modifiedTotalFee, modifiedStorageLimit, label, className, copyButtonClassName }) => {
    ...
    return (
      <>
        {label && (
          <h2 className={classNames('w-full mb-2', 'text-black font-semibold leading-tight', 'text-black')}>{label}</h2>
        )}
        ...
        <div className={classNames('relative mb-2', className)}>
          ...
          >
            {typeof opParams === 'string' ? (
              <div className={classNames('p-1', 'text-lg text-black font-normal whitespace-pre-line')}>{opParams}</div>
            ) : (
              <ReactJson
                src={opParams}
                name={null}
