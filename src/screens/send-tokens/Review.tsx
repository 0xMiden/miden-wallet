...
export const ReviewScreen: React.FC<ReviewScreenProps> = ({
  ...
  const onBackClick = () => onAction?.({ id: SendTokensActionId.GoBack });
  const [isTooltipVisible, setIsTooltipVisible] = useState(false);

  const onFeeOptionsClick = () =>
    onAction?.({
      id: SendTokensActionId.Navigate,
      step: SendTokensStep.FeeOptions
    });
  ...
  const to = useMemo(() => {
    return [ansName, shortenAddress(recipientAddress)].filter(Boolean).join('\n');
  }, [recipientAddress, ansName]);
    ...
    <div {...props} className={classNames('flex-1 flex flex-col', className)}>
      ...
      <div className="flex flex-col flex-1 p-4 gap-y-4 md:w-[460px] md:mx-auto">
        <span className="flex flex-row items-end gap-x-2 justify-center p-6">
          ...
          <p className="text-2xl leading-8">{token.name.toUpperCase()}</p>
        </span>

        <div className="flex flex-col gap-y-2">
          <span className="flex flex-row justify-between">
            <label className="text-sm text-grey-600">From</label>
            <p className="text-sm">{shortenAddress(sendAddress)}</p>
          </span>
